...
const MovieInfo = () => {
  ...
  const toggleFavorite = async (userId, movieId) => {
    try {
      ...
      if (userData?.favoritos?.includes(movieId)) {
        await updateDoc(userRef, {
          favoritos: arrayRemove(movieId),
        });
        setIsFavorited(false); // Atualiza visual
        console.log(`Filme ${movieId} removido dos favoritos.`);
      } else {
        await updateDoc(userRef, {
          favoritos: arrayUnion(movieId),
        });
        setIsFavorited(true); // Atualiza visual
  ...
  const handleAddReview = async () => {
    try {
      const user = auth.currentUser;
      if (user && reviewContent && reviewRating) {
        const reviewData = {
        ...
        const userReviewRef = doc(db, 'users', user.uid, 'movieReviews', movieId.toString());
        await setDoc(userReviewRef, { reviews: arrayUnion(reviewData) }, { merge: true });

        // Atualizar avaliações na UI
